...
2. Evaluate each on ≥ 20 different master seeds with your candidate N (instances per evaluation). Compute the per-seed score's standard deviation σ_round.
...
- **Whole-evaluation hard timeouts** (`evaluate.timeout_s` for the player, `referee.timeout_s` for the scorer): startup + N × per-instance budget + buffer. The sandbox is killed without grace at the limit; a player timeout should be a scoreable outcome your referee reports (score 0 or partial credit for completed instances) with a reason the miner can see.
