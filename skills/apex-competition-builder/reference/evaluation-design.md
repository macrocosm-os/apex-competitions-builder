...
Timeouts and deadlines are the only place wall-clock time belongs. They gate whether a response counted; they never contribute a quantity to the score, because the host's throughput varies between runs and would show up as score noise you can't distinguish from skill (SKILL.md design step 4). Measure speed, when it matters, in steps, calls, moves, or operations — units the machine can't inflate or deflate.
