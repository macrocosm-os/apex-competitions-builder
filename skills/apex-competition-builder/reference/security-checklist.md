...
- Your per-task history/trace files (SKILL.md design step 6) are part of this surface: they are collected, shipped to S3, and downloadable by the miner when the round ends. Record the conditions each task carried — never the round seed, never a per-task seed that narrows it, never anything from which a later round's tasks can be derived. The conditions are what the miner is owed; the inputs that generated them are not.
...
- **Keep the evidence.** The per-task records from SKILL.md design step 6 are what let you spot a pathway you missed after launch: a leader whose per-task rows show it winning by timing out, forfeiting, or returning nothing is visible in the history and invisible in the scalar. Without them a suspected exploit is unfalsifiable in both directions.
...
Same submission + same round input + same seed must produce the same score: pin model revisions (full SHA), dataset content hashes, dependency versions; use greedy/deterministic inference; verify data integrity at load (checksum the pool). **Keep wall-clock time out of the score entirely** — elapsed seconds, throughput, latency terms, and speed bonuses all vary with how busy and how throttled the host is, so a submission's score changes when nothing about the submission did. That is non-determinism you built in on purpose: it makes near-threshold differences unattributable and turns resubmission into a lottery worth playing. Score in host-independent units (steps, calls, moves, tokens, operations) and leave wall clock to the deadlines and timeouts, where it decides admissibility rather than value. Non-determinism is not just noise — it is a dispute you cannot win ("your evaluator is random" is unanswerable without reproducibility) and a seed-fishing vector (see evaluation-design.md). Determinism is also what makes score-affecting changes auditable: any change to scoring is a new spec `version` with a new signed digest, reviewable in git.
