...
- **Layer 1 (declarative, in your spec's `screening` block)** runs the platform's generic screener with no competition code, keyed by `submission.artifact_type`: AST bans for `code` and `archive` submissions (base forbidden sets for `socket`/network clients, file primitives beyond sanctioned paths, `pickle`/`marshal`/dynamic execution, `threading` timeout evasion, `mmap`, `signal`, dynamic-attribute escapes — extend with `extra_forbidden_*` rather than inventing your own), structural validation for `torchscript`/`onnx` (size, magic bytes, minimum-weight-data floors, code-to-weights ratio), shape limits for `json`/`csv` (`max_rows`, `max_columns`, `required_columns`, `max_json_depth`), and the host-import allowlist for `wasm` (`wasm_allowed_imports`, `wasm_max_memory_pages`). It's a tripwire, not the boundary — the sandbox is the actual defense — so it's fine that the config is visible in the spec (which the platform reviews, and which you may publish). Knobs that don't apply to your declared type are ignored, silently by the platform and loudly by `apex-dev preflight`.
- **Multi-file submissions (`artifact_type: archive`) add extraction as an attack surface.** The bundle is bounded by `submission.archive` (`max_uncompressed_mb` is the decompression-bomb ceiling, `max_files` the inode/time ceiling) and members that are absolute, contain `..`, or are symlinks/hardlinks are rejected rather than sanitised — a link is how a bundle reads a path it was never given. Set `screening.allowed_member_extensions` (usually `[".py"]`): an unrestricted bundle can smuggle a binary blob past checks aimed at Python. Every Python member is AST-screened, so a package doesn't dilute Layer 1 — but it does multiply what you're asking it to cover, and `entry_file` is the only member you actually know gets imported.
...
- Same idea for non-ML domains: accept a policy table, a parameter vector, a JSON strategy spec — anything with a closed grammar you can validate structurally in your player before serving it. These have first-class types: `artifact_type: json` and `artifact_type: csv`, with Layer-1 shape limits (`max_rows`, `max_columns`, `required_columns`, `max_json_depth`) on top of your player's own validation. Reach for them before `code`.
- **When the submission must compute, `wasm` beats `code`.** A WebAssembly module has no ambient authority: no syscalls, no filesystem, no network, no imports at all unless you grant them in `screening.wasm_allowed_imports`. That's a capability allowlist rather than a blocklist of dangerous Python you hope is exhaustive — the difference between "these are the only things it can do" and "these are the things we thought to forbid." It costs miners a toolchain step, so say so in the miner README and provide a template.
